...
ENT.TauntSounds = {
	Sound("npc_corrupted/taunt.mp3"),
}
local chaseMusic = Sound("npc_corrupted/corrupted.mp3")
...
local npc_corrupted_acquire_distance =
	CreateConVar("npc_corrupted_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which corrupted will chase a target.")

local npc_corrupted_spawn_protect =
	CreateConVar("npc_corrupted_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, corrupted will not target players or hide within 200 units of \z
	a spawn point.")

local npc_corrupted_attack_distance =
	CreateConVar("npc_corrupted_attack_distance", 80, FCVAR_NONE,
	"The reach of corrupted's attack.")

local npc_corrupted_attack_interval =
	CreateConVar("npc_corrupted_attack_interval", 0.2, FCVAR_NONE,
	"The delay between corrupted's attacks.")

local npc_corrupted_attack_force =
	CreateConVar("npc_corrupted_attack_force", 800, FCVAR_NONE,
	"The physical force of corrupted's attack. Higher values throw things \z
	farther.")

local npc_corrupted_smash_props =
	CreateConVar("npc_corrupted_smash_props", 1, FCVAR_NONE,
	"If set to 1, corrupted will punch through any props placed in their way.")

local npc_corrupted_allow_jump =
	CreateConVar("npc_corrupted_allow_jump", 1, FCVAR_NONE,
	"If set to 1, corrupted will be able to jump.")

local npc_corrupted_hiding_scan_interval =
	CreateConVar("npc_corrupted_hiding_scan_interval", 3, FCVAR_NONE,
	"corrupted will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant corrupteds are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_corrupted_hiding_repath_interval =
	CreateConVar("npc_corrupted_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to corrupted's hiding spot will be redetermined every X seconds.")

local npc_corrupted_chase_repath_interval =
	CreateConVar("npc_corrupted_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of corrupted's target will be redetermined every \z
	X seconds.")

local npc_corrupted_expensive_scan_interval =
	CreateConVar("npc_corrupted_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_corrupted_force_download =
	CreateConVar("npc_corrupted_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download corrupted resources \z
...
util.AddNetworkString("corrupted_nag")
util.AddNetworkString("corrupted_navgen")
...
local VECTOR_corrupted_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_corrupted_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_corrupted: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other corrupteds, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_corrupted"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "corruptedMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_corrupted" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why corrupted isn't working.
	net.Start("corrupted_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_corrupted: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_corrupted: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_corrupted: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_corrupted: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_corrupted: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, corrupted in pairs(ents.FindByClass("npc_corrupted")) do
		corrupted:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "corruptedNavGen", navEndGenerate)
	else
		print("npc_corrupted: nav_generate failed to initialize")
...
concommand.Add("npc_corrupted_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_corrupted: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. corrupted doesn't use
	...
	elseif isConsole then
		print("npc_corrupted: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:Initialize()
	...
	self:SetMaterial("gmod_shader_guide/example6")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_corrupted_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_corrupted_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_corrupted_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_corrupted_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. corrupted means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_corrupted_smash_props:GetBool() then continue end
...
function ENT:AtcorruptedtJumpAtTarget()
	-- No double-jumping.
	...
	local maxAttackDistance = npc_corrupted_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_corrupted_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_corrupted_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_corrupted_attack_distance:GetInt()
		...
		local repathInterval = npc_corrupted_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_corrupted_allow_jump:GetBool()
			and currentTime - self.LastJumpScan >= scanInterval
		then
			self:AtcorruptedtJumpAtTarget()
			self.LastJumpScan = currentTime
		end
	else
		local hidingScanInterval = npc_corrupted_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_corrupted_hiding_repath_interval:GetFloat()
...
local MAT_corrupted = Material("npc_corrupted/corrupted")
killicon.Add("npc_corrupted", "npc_corrupted/killicon", color_white)
language.Add("npc_corrupted", "corrupted")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_corrupted: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a corrupted.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_corrupted_music_volume =
	CreateConVar("npc_corrupted_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by corrupted. (0-1, where 0 is muted)")

-- If another corrupted comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, corrupteds do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_corrupted_PANIC_COUNT corrupteds are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of corrupted.
local MUSIC_corrupted_PANIC_COUNT = 8

local MUSIC_corrupted_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_corrupted_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_corrupted") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("corruptedPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_corrupted_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_corrupted" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_corrupted_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear corrupted taunt us.
...
local function startTimer()
	if not timer.Exists("corruptedPanicMusicUpdate") then
		timer.Create("corruptedPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
surface.CreateFont("corruptedHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("corruptedHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("corrupted is studying this map.", "corruptedHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "corruptedHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "corruptedHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "corruptedHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "corruptedHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("corrupted_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "corruptedNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. corrupted doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_corrupted_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for corrupted to figure \z
		this map out.\n\z
...
net.Receive("corrupted_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! corrupted doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet corrupted-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! corrupted doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to corrupted.\n\z
			\n\z
			If you ARE the server host, you can run npc_corrupted_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not corrupted-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_corrupted", {
	Name = "corrupted",
	Class = "npc_corrupted",
	Category = "corrupted NPC",
	AdminOnly = false
})
